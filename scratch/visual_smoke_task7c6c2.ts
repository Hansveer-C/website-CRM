...
async function structure(page: any, name: string) {
  ...
  await page.locator('#route-path').fill('/unsaved-a');
  await selectWebsite(page, 'ws-2');
  assert(await page.locator('#route-modal').count() === 0, `${name}: Site A modal survived Website B switch`);
  assert(!await page.evaluate(() => document.body.textContent?.includes('/unsaved-a') ?? false), `${name}: unsaved Site A route leaked after Website B switch`);
  assert(!await page.evaluate(async () => (await import('/src/db.ts')).mockWebsiteRoutes.some(route => route.path === '/unsaved-a')), `${name}: unsaved Site A route was persisted`);
  await page.locator('#website-structure-add-route').click();
  assert(await page.locator('#route-modal').count() === 1, `${name}: expected one fresh Site B route modal`);
  assert((await destinationIds(page)).join(',') === 'funnel-b', `${name}: Site B destinations retained stale Site A data`);
  await page.evaluate(() => (window as any).showAddRouteModal());
  assert(await page.locator('#route-modal').count() === 1, `${name}: same Website modal reuse created a duplicate`);
  await selectWebsite(page, 'ws-1');
