 ...
 */
export async function saveMessage(message: Partial<Message> & { contact_id: string }): Promise<{ success: boolean; skipped?: boolean; error?: string }> {
  ...
  const contactRes = await getContact(message.contact_id, ownerId);
  ...
  if (!saveRes.success) {
      if (saveRes.code === '23505') {
          // Unique Constraint violation - typically trigger_event_id
