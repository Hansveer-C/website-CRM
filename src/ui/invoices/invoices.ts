...
export function renderInvoicesContent(model: InvoiceModel): string {
  ...
  const total = (status: Invoice['status']) => all.filter(invoice => invoice.status === status).reduce((sum, invoice) => sum + invoice.amount, 0);
