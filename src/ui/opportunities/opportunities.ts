...
export function renderOpportunitiesContent(model: OpportunitiesModel): string {
  ...
  const columns = stages.map(summary => {
    ...
    return model.editable ? `<div class="wo-pipeline-stage-drop-target" data-stage="${escapeHtmlText(summary.stage)}" ondragover="window.allowDrop(event)" ondrop="window.drop(event, this.dataset.stage)">${stageCard}</div>` : stageCard;
  }).join('');
