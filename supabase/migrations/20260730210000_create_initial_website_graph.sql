-- Phase 0 website bootstrap: authenticated, atomic and idempotent first-site creation.
create schema if not exists private;
revoke all on schema private from public, anon, authenticated;

create table if not exists private.website_creation_receipts (
...
revoke all on private.website_creation_receipts from public, anon, authenticated;
...
begin
  ...
  -- One short transaction-scoped lock serializes all first-site attempts for this owner.
  ...
  values
    (pg_catalog.gen_random_uuid()::text, v_user_id, v_page.id, 'hero', jsonb_build_object('heading', v_business_name, 'subheading', 'Trusted service in ' || v_city, 'phone', v_phone_number), 0, '{}'::jsonb, v_now),
    (pg_catalog.gen_random_uuid()::text, v_user_id, v_page.id, 'services', jsonb_build_object('heading', 'Our Services', 'items', to_jsonb(v_services)), 1, '{}'::jsonb, v_now),
    (pg_catalog.gen_random_uuid()::text, v_user_id, v_page.id, 'cta', jsonb_build_object('heading', 'Ready to get started?', 'phone', v_phone_number), 2, '{}'::jsonb, v_now);
...
create index if not exists website_routes_funnel_id_idx on public.website_routes(funnel_id);
