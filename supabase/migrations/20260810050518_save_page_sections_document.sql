-- Atomic, tenant-scoped persistence boundary for a complete Builder page document.
...
revoke all on private.page_section_save_revisions from public, anon, authenticated;

-- Establish revision zero for documents that predate this persistence boundary.
insert into private.page_section_save_revisions(page_id, user_id, revision, document_hash)
select
  page.id,
  page.user_id,
  0,
  pg_catalog.md5(coalesce((
    ...
    where section.page_id = page.id and section.user_id = page.user_id
  ), '[]'::jsonb)::text)
from public.pages as page
on conflict (page_id) do nothing;

create or replace function public.get_page_sections_save_revision(p_page_id text)
returns jsonb
language plpgsql
security definer
stable
...
begin
  ...
  select page.user_id into v_page_owner from public.pages as page where page.id = p_page_id;
  if not found then raise sqlstate 'PT404' using message = 'Page not found'; end if;
  if v_page_owner <> v_user_id then raise sqlstate 'PT403' using message = 'Page ownership required'; end if;

  select revision, document_hash into v_revision, v_document_hash
  from private.page_section_save_revisions where page_id = p_page_id;
  if not found then
    v_revision := 0;
    v_document_hash := pg_catalog.md5('[]');
  end if;
  select count(*)::integer into v_count from public.page_sections
...
revoke all on function public.get_page_sections_save_revision(text) from public, anon;
grant execute on function public.get_page_sections_save_revision(text) to authenticated;
...
begin
  ...
  -- Serialize every replacement for this page before revision comparison.
  ...
  loop
    ...
    if coalesce(v_section->>'type', '') not in ('hero', 'proof', 'offer', 'gallery', 'form', 'faq') then
...
comment on function public.save_page_sections_document(text, jsonb, bigint, bigint) is
  'Atomically replaces one authenticated owner page section document with optimistic revision control.';
