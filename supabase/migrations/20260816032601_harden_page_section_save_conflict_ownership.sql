-- Make section-ID conflict handling an atomic ownership boundary.
-- The earlier migration remains immutable because it has already been applied.
...
begin
  ...
  -- Keep revision comparison and complete-document replacement serialized per page.
...
comment on function public.save_page_sections_document(text, jsonb, bigint, bigint) is
  'Atomically replaces one authenticated owner page section document with optimistic revision and conflict ownership control.';
