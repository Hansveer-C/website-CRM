...
// src/test_standardized_timeline.ts
var contactId = "c1";
...
var relevantEvents = mockEventLogs.filter((log) => log.payload.contact_id === contactId);
var unifiedTimeline = [
  ...activities.map((a) => ({
    type: "activity",
    reference_id: a.id,
  ...
  ...relevantEvents.map((e) => ({
    type: e.event_name === "form_submitted" ? "form_submission" : "event",
    reference_id: e.id,
    contact_id: contactId,
...
var isStandardized = unifiedTimeline.every(
  (item) => item.type && item.reference_id && item.contact_id && item.content && item.created_at
);
