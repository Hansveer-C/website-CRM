...
async function verifyV3() {
  ...
  try {
    ...
    db2.prepare("DELETE FROM contacts WHERE email = ?").run(email);
    db2.prepare("DELETE FROM opportunities WHERE source = 'v3_test'").run();
    ...
    try {
      const res2 = await createLead({
        name: "V3 Second User",
        // Attempting to "overwrite" or create new
        phone,
        email,
        source: "v3_test"
      });
    ...
    const contactCount = db2.prepare("SELECT count(*) as total FROM contacts WHERE email = ?").get(email);
    const oppCount = db2.prepare("SELECT count(*) as total FROM opportunities WHERE contact_id = ?").get(res1.contactId);
